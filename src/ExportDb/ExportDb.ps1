...
try {
    ...
    if ($export.status -eq "InProgress") {
        ...
        if ($status.status -eq "Succeeded") {
            ...
            Write-Warning "The database export has not been successful or the script has timedout."
            Write-Host "##vso[task.logissue type=error]The database export has not been successful or the script has timedout."
            Write-Error "The database export has not been successful or the script has timedout." -ErrorAction Stop
            exit 1
        }
    }
    else {
